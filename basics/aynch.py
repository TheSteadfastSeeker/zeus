...
from tools.configuration import OllamaConfiguration as Configuration

configuration = Configuration()
llm = configuration.get_llm(temperature=0)

async def asimple_calculator_llm() -> list[str]:
    start = time.time()
    result1 = llm.ainvoke("Is there life in Goldilock's zone of black hole.")
    result2 = llm.ainvoke("write a song about never giving up.")
    result3 = llm.ainvoke("describe the deepest part of the ocean.")
    end = time.time()
    print(f" reached ahere in... {end-start}") # reached ahere in... 5.0067901611328125e-06
    return [(await result1).content, (await result2).content, (await result3).content]

def simple_calculator_llm() -> list[str]:
    start = time.time()
    result1 = llm.invoke("Is there life in Goldilock's zone of black hole.")
...
async def main():
    result5 = await asimple_calculator_llm()
